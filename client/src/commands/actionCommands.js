    ...
    bot.action('finish_order', async (ctx) => {
        ...
        if(user.pnumber === ""){
            ...
            let result = await Tickets.addTicket({
                ...
                date: `${date.getDate()}/${date.getMonth()+1}/${date.getFullYear()} ${(date.getHours()-1) < 0 ? 23 : data.getHours()-1}:${date.getMinutes() < 10 ? '0'+date.getMinutes() : date.getMinutes()}:${date.getSeconds()}`,
            });
